<template>
    <div class="tailor-personality">
        <el-breadcrumb separator="/" style="margin-bottom:20px;">
            ...
            <el-breadcrumb-item>编辑尺码模板</el-breadcrumb-item>
        </el-breadcrumb>
        <div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.$q({
            url: "/bg_admin/size/editTemplate",
            data,
            method: "post"
          }).then(res => {
    ...
    query(){
        this.$q({
            url:"/bg_admin/size/getTemplateDetail",
            params:{id:this.id}
        }).then(res=>{
            this.form.template_name = res.template_name;
           this.last_item_ids= this.form.item_ids = res.item_ids;

            var val = res.values;
            val.forEach(v=>{
                this.form.attrs.push(v.size_name);
                this.form.table.push(v.items);
            })
        })
    }
  },
