...
export default {
  ...
  methods: {
     ...
     submit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.$q({
            url: "/bg_admin/bg_management/editTemplate",
            data:this.form,
            method: "post"
          }).then(res => {
    ...
    fetch_areas_data(array){
        if(!Array.isArray(array) || array.length ===0){
            return;
        }
        this.areas = array.map(v=>{
            v.province =[];
            v.province_code =[];
           v.areas.forEach(val=>{
               v.province.push(val.province_name);
               v.province_code.push(val.province_code);
           })
            return v;
        })
    },
    query(){
        this.$q({
           url:"/bg_admin/bg_management/getTemplateDetail",
           params:{id:this.id} 
        }).then(res=>{
            this.form.mname= res.mname;
            this.form.default_first=res.default_first;
            this.form.default_first_cost=res.default_first_cost;
            this.form.default_second=res.default_second;
            this.form.default_second_cost=res.default_second_cost;
            this.form.type_id =res.type_id;

            this.fetch_areas_data(res.area_temp);  
