<template>
    <div class="tailor-edit-personality">
        <el-breadcrumb separator="/" style="margin-bottom:20px;">
            <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
            <el-breadcrumb-item>编辑个性化工艺</el-breadcrumb-item>
        </el-breadcrumb>
        <div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.$q({
            url: "/bg_admin/personalization/editPersonalization",
            data,
            method: "post"
          }).then(res => {
    ...
    query() {
      this.$q({
        url: "/bg_admin/personalization/getPersonalizationDetail",
        params: { id: this.id }
      }).then(res => {
        console.log(res);
