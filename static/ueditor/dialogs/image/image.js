    ...
    function setTabFocus(id) {
        ...
        switch (id) {
            ...
            case 'online':
                ...
                onlineImage.reset();
                $("#pcheckall").append("<select style='margin-right:10px;' id='sort'><option value='1'>时间降序</option><option value='2'>时间升序</option><option value='3'>名字降序</option><option value='4'>名字升序</option></select><label>全选<input style='position:relative;bottom:-2px;'id='checkall' type='checkbox'></label>");
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                if (li.tagName.toLowerCase() == 'li' && !li.hasAttribute("data-path")) {
            ...
            domUtils.on(this.container, 'dblclick', function (e) {
                var target = e.target || e.srcElement,
                    li = target.parentNode;
                if(li.hasAttribute("data-path")) {
                    _this.dirpath = li.getAttribute("data-path")
                    _this.reset();
                    $("#sort").val(1);
                    _this.sort = 1;
        ...
        pushData: function (list) {
			var diricon = editor.getOpt("imageManagerDirIcon");
            var pdiricon = editor.getOpt("imageManagerPDirIcon");
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    domUtils.on(img, 'load', (function(image){
			             return function(){
			                 _this.scale(image, image.parentNode.offsetWidth, image.parentNode.offsetHeight-27);
			             }
                    })(img));
                    img.width = 113;
                    if(list[i].type == "dir") {
                        img.setAttribute('src', diricon);
                        item.setAttribute('data-path', list[i].url);
                        item.setAttribute('class', "ui-state-disabled");
					} else if(list[i].type == "pdir") {
                        img.setAttribute('src', pdiricon);
                        item.setAttribute('data-path', list[i].url);
                        item.setAttribute('class', "ui-state-disabled");
					} else {
					    item.setAttribute("name", list[i].name);
                        item.setAttribute("time", list[i].time);
                        if($("#checkall").is(":checked")) {
                           item.setAttribute('class', "sortable selected");
                        } else {
                           item.setAttribute("class", "sortable"); 
                        }
                    ...
                    name.setAttribute("title", list[i].name);
                    this.list.insertBefore(item, this.clearFloat);
                }
            }
            $("ul.list").sortable({items: ".sortable"});
